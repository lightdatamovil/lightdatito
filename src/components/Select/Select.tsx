...
const Select = ({ id, label, value, onChange, disabled = false, className = "", children }: SelectProps) => {
    return (
        <div className="w-full relative pt-2.5">
            <select id={id} value={value} onChange={onChange} disabled={disabled} className={`peer select_select h-[45px] w-full border border-tito-border-gray bg-tito-bg-primary rounded-xl px-4 text-[12px] focus:outline-none focus:border-tito-lila ${className}`}>
                {children}
            </select>
